...
- 100 patients, same schema as full MIMIC-IV
- No CITI training required for the demo
- Download: [physionet.org/content/mimic-iv-demo/2.2](https://physionet.org/content/mimic-iv-demo/2.2/)
- Or via Kaggle: search `mimic-iv-clinical-database-demo-2-2`
...
```
PatientReadmissionRiskEngine/
├── hosp/                        # MIMIC-IV Demo tables
│   ├── admissions.csv
│   ├── patients.csv
│   └── diagnoses_icd.csv
├── data/
│   └── readmission_cohort.csv   # Generated by notebook
├── ClinicalRisk.ipynb           # Main notebook (all cells)
└── README.md
```
...
1. Download MIMIC-IV Demo and place CSVs in `hosp/`
2. Open `ClinicalRisk.ipynb` in Jupyter
3. Run all cells top to bottom (`Kernel → Restart & Run All`)
4. Final output: cohort summary, AUC scores, SHAP plot, per-patient uncertainty table

**Requirements:**
```
pip install pandas numpy scikit-learn torch shap matplotlib
```
